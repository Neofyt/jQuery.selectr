...
;(function($) {
	...
	$.fn.selectr = function(options, callback) {
		...
		return this.each(function() {
			...
			var table = $(this),
				...
				boxTempl = '<input type="checkbox" class="'+ o.selectr +'" />';

			// Creating the radioboxes
			if (o.position === "right"){
				o.checkMax === 0 ?
					head.find("tr").append('<th>' + boxTempl + '</th>') :
					head.find("tr").append('<th></th>');
				body.find("tr").each(function() {
					$(this).append('<td>' + boxTempl + '</td>');
				});
			} else {
				o.checkMax === 0 ?
					head.find("tr").prepend('<th>' + boxTempl + '</th>') :
					head.find("tr").prepend('<th></th>');
				body.find("tr").each(function() {
					$(this).prepend('<td>' + boxTempl + '</td>');
				});
			}
	...
	$.fn.selectr.updateCount = function(table, foot, total, o, list, head){
		...
		if (list === true){ $.fn.selectr.checkedRows(table, o, head); }
	};
	...
	$.fn.selectr.listToCheck = function(table, o, callback, headRad, bodyRad, foot){
		...
		if (listLength !== 0){
			listLength === bodyRad.length ?
				headRad.prop("checked", true) :
				headRad.prop("indeterminate", true);

			// If the provided list is composed of ids...
			if (isNaN(o.list[0])){ // IDs can't be composed of just integers
				for (i=0; i<listLength; i++){
					table.find("#" + o.list[i]).find("." + o.selectr).prop("checked", true).parent().parent().addClass(o.highlight);
				}

			// ... or row numbers
			} else {
				for (i=0; i<listLength; i++){
					table.find("tbody tr:nth-child(" + o.list[i] + ")").find("." + o.selectr).prop("checked", true).parent().parent().addClass(o.highlight);
				}
			}
		}
